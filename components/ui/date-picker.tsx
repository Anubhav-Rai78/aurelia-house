...
>(({ className, align = "start", sideOffset = 8, ...props }, ref) => (
  <PopoverPrimitive.Portal>
    <PopoverPrimitive.Content
      ...
      className={cn(
        "z-[90] w-auto rounded border border-forest/15 bg-ivory p-4 text-forest shadow-xl outline-none transition-all duration-200 animate-fadeIn",
        className
      )}
      {...props}
    />
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        weekdays: "flex mb-1",
        weekday:
          "text-forest/50 rounded-md w-9 font-sans text-[11px] font-medium uppercase tracking-[0.08em] text-center",
        week: "flex w-full mt-1",
        day: cn(
          "h-9 w-9 p-0 font-sans text-[14px] font-normal text-forest aria-selected:opacity-100 hover:bg-sand/50 transition-colors flex items-center justify-center rounded-[2px]"
        ),
...
export function DatePicker({
  ...
  return (
    <div className={cn("relative w-full", className)}>
      {label && <span className="block text-label text-forest/60 mb-1">{label}</span>}
