...
// (Aurelia House contact/reservation API routes, etc.). It centralises:
...
export async function apiClient<T>(
  ...
  try {
    ...
    // 401 — the endpoint rejected the request. There is no login flow on the
    // public hotel site, so surface the status plainly instead of redirecting.
...
export function registerChunkLoadRecovery(): void {
  ...
  const handler = (event: PromiseRejectionEvent) => {
    ...
    if (/Loading chunk .* failed/i.test(message)) {
      ...
      if (safeGetStorage("aurelia:chunk-reload") !== currentUrl) {
        safeSetStorage("aurelia:chunk-reload", currentUrl);
